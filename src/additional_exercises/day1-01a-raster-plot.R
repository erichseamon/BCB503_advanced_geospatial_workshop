...
#package with customized coloring schemes. It also covers how to 
#layer a raster on top of a hillshade to produce an eloquent map. 
#We will continue working with the Digital Surface Model (DSM) raster
#for the NEON Harvard Forest Field Site. 

## Plotting Data Using Breaks
#In the previous episode, we viewed our data using a continuous color ramp. 
#For clarity and visibility of the plot, we may prefer to view the data 
#"symbolized" or colored according to ranges of values. This is comparable 
#to a "classified" map. To do this, we need to tell `ggplot` how many 
#groups to break our data into, and where those breaks should be. 
#To make these decisions, it is useful to first explore the distribution 
#of the data using a bar plot. To begin with, we will use 
#`dplyr`'s `mutate()` function combined with `cut()` to split 
...
#To implement this we will give `mutate()` a numeric vector of 
#break points insteadof the number of breaks we want.
...
#Note that when we assign break values a set of 4 values will 
#result in 3 bins of data. The bin intervals are shown using 
#`(` to mean exclusive and `]` to mean inclusive. 
#For example: `(305, 342]` means "from 306 through 342".
...
#And we can get the count of values in each group in the same 
#way we did before:
...
#We can use those groups to plot our raster data, with each 
#group being a different color:
...
#If we need to create multiple plots using the same color palette, 
#we can create an R object (`my_col`) for the set of colors that 
#we want to use. We can then quickly change the palette across 
#all plots by modifying the `my_col` object, rather than each individual plot.
...
#passing `element_blank()` to the relevant part of the `theme()` function.
...
#We can layer a raster on top of a hillshade raster for the same 
#area, and use a transparency factor to create a 3-dimensional 
#shaded effect. A hillshade is a raster that maps the shadows 
#and texture that you would see from above when viewing terrain.
...
#We can layer another raster on top of our hillshade by adding 
#another call to the `geom_raster()` function. Let's overlay 
#`DSM_HARV` on top of the `hill_HARV`.
...
#Use the files in the `NEON_RemoteSensing/SJER/` directory 
#to create a Digital Terrain Model map and Digital Surface Model map of the 
#San Joaquin ExperimentalRange field site.
