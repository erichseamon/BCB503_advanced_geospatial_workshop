...
#data contains imagery data, each band may represent reflectance for a 
#different wavelength (color or type of light) or set of wavelengths - 
#for example red, green and blue. A multi-band raster may two or more 
#bands or layers of data collected at different times for the same extent 
#(region) and of the same resolution. For this episode, we will work with 
#a time series of normalized difference vegetation index (NDVI) and RGB 
#data from the Harvard Forest site.  We introduced the concepts of NDVI and 
#RGB data previously.

#In this episode, we will:

#1. Import NDVI data in GeoTIFF format.
#2. Import, explore and plot NDVI data derived for several dates throughout 
#the year.
...
#available from Landsat data. The RGB directory contains RGB images for 
#each time period that NDVI is available.

### Getting Started

#In this episode, we will use the `raster`, `rgdal`, `reshape`, and 
#`scales` packages. Make sure you have them loaded.
...
#will use the syntax `pattern=".tif$"`. If we specify `full.names = TRUE`, 
#the full path for each file will be added to the list.

## Data Tip
#In the pattern above, the `$` character represents the end of a line. 
#Using it ensures that our pattern will only match files that end in `.tif`. 
...
#It's a good idea to look at the file names that matched our search 
#to make sure they meet our expectations. 
...
#specifically `yres()` and `xres()`).

crs(NDVI_HARV_stack)


#The CRS for our stack is `+proj=utm +zone=19 +ellps=WGS84 +units=m +no_defs`. 
#The CRS is in UTM Zone 19. If you have completed the previous episodes in
#this workshop, you may have noticed that the UTM zone for the NEON collected 
...
#multiple UTM zones. When the data are processed, the zone in which 
#the majority of the data cover is the zone which is used for the final 
#CRS. Thus, our field site at Harvard Forest is located in UTM Zone 18, 
#but the Landsat data is in a CRS of UTM Zone 19.
...
## Plotting Time Series Data
 ...
 #the `ggplot()` command to create a multi-panelled plot showing each band 
 #in our RasterStack. First we need to create a data frame object. 
 #Because there are multiple bands in our data, we will reshape (or "melt")
 #the data so that we have a single column with the NDVI observations. 
...
#The metadata for this NDVI data specifies a scale factor: 10,000. 
#A scale factor is sometimes used to maintain smaller file sizes by 
...
#After applying our scale factor, we can recreate our plot using the 
#same code we used above.
...
#spring, continues to grow throughout the summer, and begins to change colors 
#and senesce in the fall through winter. Do you notice anything that seems 
#unusual about the patterns of greening and browning observed in the plots 
#above?
...
#with Julian day, check out the NEON Data Skills tutorial.
...
#Next we will use histograms to explore the distribution of NDVI values 
#stored in each raster.
...
#It seems like things get green in the spring and summer like we expect, but 
#the data at Julian days 277 and 293 are unusual. It appears as if the 
#vegetation got green in the spring, but then died back only to get 
#green again towards the end of the year. Is this right?

### Explore Unusual Data Patterns
#The NDVI data that we are using comes from 2011, perhaps a strong freeze 
#around Julian day 277 could cause a vegetation to senesce early, 
#however in the eastern United States, it seems unusual that it would 
#proceed to green up again shortly thereafter.
...
#The `date` column is currently coded as a factor. We want to be able to 
#treat it as a date,so we will use the `as.Date()` function to convert it. 
#We need to tell R what format the data is in. Our dates are YYY-MM-DD, 
#which is represented by R as `%Y-%m-%d`.
...
#Now we can plot the air temperature (the `airt` column) by Julian day 
#(the `jd` column):
...
#summer or early fall time period that might account for patterns seen 
#in the NDVI data.  Let's have a look at the source Landsat imagery that 
#was partially used used to derive our NDVI rasters to try to understand what appear to be outlier NDVI values.
...
#plots for Julian days 133 and 197 (shown above). Does the RGB imagery from these two days explain the low NDVI values observed on these days?
...
#we see that most of the image is filled with clouds. The very low NDVI values resulted from cloud cover — a common challenge that we encounter when working with satellite remote
#sensing imagery.
